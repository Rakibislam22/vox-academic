...
export default function RecentReadsView({ onSelectDocument, documents: initialDocs }: RecentReadsViewProps) {
    ...
    if (loading) return <div className="text-slate-400">Loading recent reads...</div>;
        ...
        <div className="w-full text-slate-200">
            ...
            {documents.length === 0 ? (
                ...
                <div className="flex flex-col gap-3">
                    {documents.map(doc => (
                        <div key={doc.id} className="flex items-center justify-between rounded-xl border border-white/10 bg-white/5 p-4 transition-colors hover:bg-white/10">
                            <div className="flex flex-col gap-1 overflow-hidden">
                                <h3 className="truncate text-base font-semibold text-white">{doc.title}</h3>
                                <span className="text-xs text-slate-400">
                                    Last read: {new Date(doc.lastReadAt || doc.createdAt).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                                </span>
                            </div>
                            <button
