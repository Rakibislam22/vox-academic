...
export default function SettingsView() {
    ...
    const handlePurge = async (id: string) => {
        if (!window.confirm('Are you sure you want to permanently delete this document? This will remove the file from cloud storage and cannot be undone.')) return;
    ...
    if (loading) return <div className="text-slate-400 flex items-center gap-2"><Loader2 className="animate-spin w-4 h-4" /> Fetching storage data...</div>;
        ...
        <div className="w-full text-slate-200 animate-fade-in">
            ...
            <div className="overflow-hidden rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl shadow-lg">
                <div className="border-b border-white/10 bg-black/20 px-6 py-4">
                    ...
                    <p className="text-xs text-slate-400 mt-1">Manage your uploaded PDFs and generated AI insights.</p>
                </div>
                        ...
                        <tbody className="divide-y divide-white/5">
                            {documents.map(doc => (
                                <tr key={doc.id} className="transition-colors hover:bg-white/5">
                                    <td className="px-6 py-4 font-medium text-white break-words max-w-[200px] sm:max-w-xs">{doc.title}</td>
                                    ...
                                    <td className="px-6 py-4 text-right whitespace-nowrap">
                                        ...
                                        >
                                            ...
                                            <span>{deletingIds.has(doc.id) ? 'Purging...' : 'Purge Document'}</span>
                                        </button>
                                    </td>
                                </tr>
                            ))}
