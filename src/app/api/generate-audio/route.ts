...
export async function POST(request: Request) {
  ...
  if (!token) {
    return buildJsonError(500, 'Server configuration error: missing HF_ACCESS_TOKEN', {
  ...
  try {
    ...
    if (upstreamResponse.status === 429) {
      ...
      return buildJsonError(429, 'Hugging Face rate limit exceeded. Please retry shortly.', {
    ...
    if (!upstreamResponse.ok) {
      ...
      return buildJsonError(502, 'Hugging Face upstream request failed', {
    ...
    if (audioBytes.byteLength === 0) {
      return buildJsonError(502, 'Hugging Face returned an empty audio payload', {
    ...
    if (error instanceof Error && error.name === 'AbortError') {
      return buildJsonError(504, 'Hugging Face request timed out', {
    ...
    if (isNetworkFetchFailure(error)) {
      ...
      return buildJsonError(503, 'Hugging Face is offline or unreachable', {
