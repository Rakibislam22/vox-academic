...
export default function ControlBar({
  ...
  const voices = ['Aura - Natural', 'Nova - Confident', 'Echo - Calm', 'Ember - Energetic'];
      ...
      <div className="flex items-center gap-3 sm:gap-4 min-w-0 w-full lg:w-auto">
        ...
        <div className="flex-1 min-w-0 w-full lg:w-64 xl:w-md">
          <input
            ...
            style={{
              background: `linear-gradient(to right, #1a8cff 0%, #1a8cff ${
            ...
            <div className="absolute bottom-12 right-0 bg-white/4 backdrop-blur-xl border border-white/10 rounded-xl shadow-lg z-modal overflow-hidden min-w-24">
              {speeds.map((speed) => (
                <button
                  ...
                  className={`block w-full px-4 py-2 text-left text-sm border-b border-white/5 hover:bg-white/10 active:scale-95 transition-transform ${
                    playbackSpeed === speed ? 'text-electric-blue font-semibold' : ''
                  }`}
                >
                  {speed}x
                </button>
            ...
            <div className="absolute bottom-12 right-0 bg-white/4 backdrop-blur-xl border border-white/10 rounded-xl shadow-lg z-modal overflow-hidden max-w-xs min-w-40">
              {voices.map((voice) => (
                <button
                  ...
                  className={`block w-full px-4 py-2 text-left text-sm border-b border-white/5 hover:bg-white/10 active:scale-95 transition-transform whitespace-nowrap ${
                    selectedVoice === voice ? 'text-electric-blue font-semibold' : ''
                  }`}
                >
                  {voice}
                </button>
