...
function scoreVoice(voice: SpeechSynthesisVoice) {
  ...
  const preferredIndex = preferredNames.findIndex((name) =>
...
export function useSpeechSynthesisSync(cleanedTextForSpeech: string): SpeechSyncState {
  ...
  const speakFromWordIndex = useCallback(
    (wordIndex: number) => {
      ...
      const safeWordIndex = Math.max(
        0,
        Math.min(wordIndex, tokens.filter((token) => token.isWord).length - 1),
      );
      const startingToken = tokens.find(
        (token) => token.isWord && token.wordIndex === safeWordIndex,
      );
      ...
      const selectedVoice = selectPremiumVoice(
